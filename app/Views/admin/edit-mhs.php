    ...
    function load_kelas() {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                let element = document.getElementById("mhs-kelas");
                element.value = <?= $mhs['kelas_mhs'] ?>;
            };
        };
    ...
    function load_bus() {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                let element = document.getElementById("bus-id");
                element.value = <?= $mhs['id_bus'] ?>;
            };
        };
    ...
    function load_kamar() {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                let element = document.getElementById("kamar-id");
                element.value = <?= $mhs['id_kamar'] ?>;
            };
        };
    ...
    function load_kelompok() {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                let element = document.getElementById("kelompok-id");
                element.value = <?= $mhs['id_kelompok'] ?>;
            };
        };
...
<div class="container w-75 mx-auto mt-2">
    <form action="/admin/update/mahasiswa" method="post">
        <div class="mb-3">
            <label for="mhs-npm" class="form-label text-light">NPM Mahasiswa</label>
            <input type="text" class="form-control" id="mhs-npm" name="mhs-npm" value="<?= $mhs['npm_mhs'] ?>" readonly>
        </div>
        <div class="mb-3">
            <label for="mhs-name" class="form-label text-light">Nama Mahasiswa</label>
            <input type="text" class="form-control" id="mhs-name" name="mhs-name" value="<?= $mhs['nama_mhs'] ?>">
        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <input class="form-control mt-2" type="number" name="seat-number" id="seat-number" placeholder="Nomor Seat" value="<?= $mhs['no_seat'] ?>">
        </div>
        <div class="mb-3">
